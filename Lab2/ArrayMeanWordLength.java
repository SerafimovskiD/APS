...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr)
    {
        ...
        float average;
        average=(float)suma/arr.getSize();
        String closestWord=arr.get(0);
        float closestWordDiff;
        closestWordDiff=Math.abs(closestWord.length()-average);
        for(int i=0;i<arr.getSize();i++)
        {
            String s= arr.get(i);
            float diff;
            diff=Math.abs(s.length()-average);
            if(diff<closestWordDiff || (diff==closestWordDiff && s.length()>closestWord.length()))
            {
                closestWord=s;
                closestWordDiff=diff;
            }
        }
        return closestWord;
